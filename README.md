...
- **Catalog**: cat and animal volunteer work around Seattle and online (camera-trap classification on Zooniverse and Instant Wild, wildlife logging on iNaturalist and eBird, Best Friends' at-home projects): Seattle Humane's youth projects and teen programs, RASKC's Shelter Helper from age 9 with a parent, PAWS Preteen Workshops and at-home projects, The NOAH Center's Kid's Day, Purrfect Pals from 13, and family fostering with Seattle Area Feline Rescue, the Seattle Animal Shelter and Homeward Pet. What fits now leads; what opens at 13 or 16 is badged *From age N*. Cards show the area (South · Kent, Far north · Arlington…) and filter by it. Marking one *Interested* opens the next step: the how-to and a prefilled introduction email; once applied, the dashboard nudges if two weeks pass with no reply. Each carries its source URL and check date, and shows *Fits now*, *With an adult*, or *Age not stated* against the volunteer's age. *Log hours* or *Plan it* on a card creates the organization and work item behind it on first use, so logging is one step; the Log hours dialog has the same picker.
...
- **Rewards**: points for the doing (10 per hour, a few more for a reflection, a photo, a plan carried out, a memo) set a level from Starter to Star; badges for milestones (first hours, 5/10/25/50/100/250 hours, three months in a row, a work item completed, first photo…) are pinned when first reached; a parent-curated shelf lets her claim rewards with points, which never lowers her level. No streaks that punish a missed week, no leaderboard.
- **Reports**: a summary; **verification letters**, one page per organization with the hours listed and supervisor and parent signature lines; or the **monthly hours log** in the format Seattle Humane's own form uses (date, time in, description, time out, total time, signature per row, twelve rows, landscape), filled in from the tracker. Entries carry time in and out and a "signed off" mark, so the log shows which rows still need a signature.
